...
student_names <- c("Arun", "Rahul", "Arnav", "Raj", "Suresh")
...
for(student_idx in 1:num_students){
  student_name <- student_names[student_idx]
  
  total_marks <- sum(course_marks[student_idx, ])
  ...
  student_record <- list(name=student_name, marks=course_marks[student_idx, ],
