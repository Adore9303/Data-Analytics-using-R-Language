...
students<- data.frame(
  ...
  Scores = sample(60:100, num_students,
                  replace = TRUE), Attendance =
    sample(60:100, num_students, replace =TRUE)
...
bar_plot <- ggplot(students, aes(x = Name,
                                 y = Scores)) + geom_bar(stat = "identity",
                                                         fill = "orange") + theme(axis.text.x =
                                                                                    element_text(angle = 45, hjust = 1)) +
  labs(title = "Bar Plot - Distribution of Scores", x = "Student Names", y ="Scores")

students$Time <- seq_len(num_students)
line_plot <- ggplot(students, aes(x = Time, y =
                                    Scores, group = 1)) +geom_line(color =
                                                                     "purple") +
  labs(title = "Line Plot - Trend of Scores Over Time", x = "Time", y ="Scores")

histogram_plot <- ggplot(students, aes(x =
                                         Scores)) + geom_histogram(binwidth = 10,
                                                                   position = "identity", color = "brown") +
